...
function mapPeerDependenciesUmd(dependencies) {
  return Object.keys(dependencies || {}).reduce((acc, key) => {
    switch (key) {
      case "dexie":
        acc.push({ dexie: "Dexie" });
        break;
      case "rxjs":
        acc.push({ rxjs: "rxjs" });
        acc.push({ "rxjs/operators": ["rxjs", "operators"] });
        break;

      default:
        acc.push(mapDependenciesRE({ [key]: null })[0]);
    }
    return acc;
  }, []);
}
...
const configLib = (packagePath) => {
  ...
  const config = {
    ...
    inlinedLibraries: Object.keys(packageJson.devDependencies || {}).filter(
      (x) => !Object.keys(packageJson.peerDependencies || {}).includes(x)
    ),
    ...
    peerDependenciesMappedUmdImports: mapPeerDependenciesUmd(
