...
export class PopulateTable<T, TKey, B extends boolean, K extends string> implements TableMapPopulate {

    /**
     * Create an Populated Collection of this table.
     */
    public toCollection(): PopulatedCollection<T, TKey, B, K> {
        return new PopulatedCollection(this._db, this._table, this._table.toCollection(), this._keys, this._options);
    }
    ...
    get(equalityCriterias: { [key: string]: any; }): Promise<Populated<T, B, K> | undefined>;
    ...
    where(equalityCriterias: { [key: string]: IndexableType; }): PopulatedCollection<T, TKey, B, K>;
    ...
    public where(
        ...
        if (whereClauseOrCollection instanceof CollectionExt) {
            ...
            return new PopulatedCollection(this._db, this._table, collection, this._keys, this._options);
    ...
    public orderBy(index: string | string[]): PopulatedCollection<T, TKey, B, K> {
        ...
        const populatedCollection = new PopulatedCollection(this._db, this._table, collection, this._keys, this._options);
    ...
    public filter: (...args: Parameters<Table<T, TKey>['filter']>) => PopulatedCollection<T, TKey, B, K>;
    public offset: (...args: Parameters<Table<T, TKey>['offset']>) => PopulatedCollection<T, TKey, B, K>;
    public limit: (...args: Parameters<Table<T, TKey>['limit']>) => PopulatedCollection<T, TKey, B, K>;
    public reverse: (...args: Parameters<Table<T, TKey>['reverse']>) => PopulatedCollection<T, TKey, B, K>;
