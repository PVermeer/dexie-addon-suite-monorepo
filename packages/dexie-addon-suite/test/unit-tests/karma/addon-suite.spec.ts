...
describe("dexie-addon-suite addon-suite.spec", () => {
    ...
    databasesPositive.forEach((database, _i) => {
      ...
      describe(database.desc, () => {
        ...
        beforeEach(async () => {
          ...
          friend = friends[0];
          [ids, clubIds, groupIds, hairColorIds, themeIds, styleIds] =
            await Promise.all([
          ...
          friendExpectedPop = new Friend(friend) as Populated<
            Friend,
            false,
            string
          >;
          ...
          friendExpectedPop.group = groups.map((x, i) => {
            x.id = groupIds[i];
            return x;
          })[1] as any;
          friendExpectedPop.hairColor = hairColors.map((x, i) => {
            x.id = hairColorIds[i];
            return x;
          })[1] as any;

          friendExpectedPop.memberOf[1]!.theme = themes.map((x, i) => {
            x.id = themeIds[i];
            return x;
          })[1] as any;
          friendExpectedPop.memberOf[1]!.theme!.style = styles.map((x, i) => {
            x.id = styleIds[i];
            return x;
          })[1] as any;
        ...
        describe("Rxjs", () => {
          ...
          it("should be able to get an observable", async () => {
            const friendObs = await firstValueFrom(
              db.friends.$.get(id).pipe(take(1))
            );
        ...
        describe("Populate - Observable", () => {
          ...
          it("should be the same result", async () => {
            expect(
              db.friends.$.populate() instanceof PopulateTableObservable
            ).toBeTrue();
            expect(
              db.friends.populate().$ instanceof PopulateTableObservable
            ).toBeTrue();
          ...
          const methods = [
            ...
            },
            {
              ...
              method: (dbInput: typeof db) =>
                dbInput.friends.$.populate([
                  "hasFriends",
                  "memberOf",
                  "group",
                  "hairColor",
          ...
          methods.forEach((_method, _j) => {
            ...
            describe(_method.desc, () => {
              ...
              it("should emit the correct value", async () => {
                const method = _method.method(db);
                const getFriend = await firstValueFrom(
                  method.get(id).pipe(take(1))
                );
              ...
              it("should not emit when no changes", async () => {
                ...
                const newIds = await db.friends.bulkAdd(newFriends, {
                  allKeys: true,
                });
              ...
              it("should emit when updating a nested populated id, then update update this record", async () => {
                ...
                const newGroupIds = await db.groups.bulkAdd(newGroups, {
                  allKeys: true,
                });
              ...
              it("should not emit when other populated properties are updated", async () => {
                ...
                const newClubIds = await db.clubs.bulkAdd(mockClubs(), {
                  allKeys: true,
                });
                const newThemeIds = await db.themes.bulkAdd(mockThemes(), {
                  allKeys: true,
                });
                const newGroupIds = await db.groups.bulkAdd(mockGroups(), {
                  allKeys: true,
                });
              ...
              describe("Array methods", () => {
                it("should be an array", async () => {
                  const method = _method.method(db);
                  const getFriends1 = await firstValueFrom(
                    method.toArray().pipe(take(1))
                  );
                  expect(Array.isArray(getFriends1)).toBeTrue();
                  const getFriends2 = await firstValueFrom(
                ...
                it("should populate an entire table", async () => {
                  ...
                  getFriendsPromises.forEach((getFriends) => {
                    ...
                    expect(
                      getFriends.some((x) => x.hasFriends.length)
                    ).toBeTrue();
                    expect(
                      getFriends.some((x) => x.memberOf.length)
                    ).toBeTrue();
                    expect(
                      getFriends.some((x) => x.group instanceof Group)
                    ).toBeTrue();
                    getFriends.forEach((x) => {
                      expect(
                        x.hasFriends.every((y) => y instanceof Friend)
                      ).toBeTrue();
                      expect(
                        x.memberOf.every((y) => y instanceof Club)
                      ).toBeTrue();
                      x.hasFriends.forEach((y) =>
                        expect(
                          y?.hasFriends.every((z) => z instanceof Friend)
                        ).toBeTrue()
                      );
                    });
                  });
                ...
                it("should order by age and populate", async () => {
                  const method = _method.method(db);
                  const orderedFriends = await firstValueFrom(
                    method.orderBy("age").toArray().pipe(take(1))
                  );
              ...
              describe("Indices", () => {
                it("should index memberOf", async () => {
                  const method = _method.method(db);
                  const getFriend = await firstValueFrom(
                    method.get({ group: groupIds[1] }).pipe(take(1))
                  );
                  expect(getFriend).toEqual(friendExpectedPop);
                });
                it("should multiIndex memberOf", async () => {
                  const method = _method.method(db);
                  const getFriend = await firstValueFrom(
                    method.get({ memberOf: clubIds[1] }).pipe(take(1))
                  );
                  expect(getFriend).toEqual(friendExpectedPop);
                });
              });
              describe("Compound index", () => {
                it("should index [id+group]", async () => {
                  const method = _method.method(db);
                  const getFriend = await firstValueFrom(
                    method.get({ id, group: groupIds[1] }).pipe(take(1))
                  );
                  expect(getFriend).toEqual(friendExpectedPop);

                  const [getFriendWhere] = await firstValueFrom(
                    method
                      .where("[id+group]")
          ...
          it("should be shallow", async () => {
            const friend = await firstValueFrom(
              db.friends.populate({ shallow: true }).$.get(id)
            );
            expect(
              friend?.hasFriends.every((x) => x instanceof Friend)
            ).toBeTrue();
            expect(
              friend?.hasFriends.every(
                (x) =>
                  typeof x?.hasFriends.some((y: any) => !(y instanceof Friend))
              )
            ).toBeTrue();
          ...
          it("should be partially shallow", async () => {
            const friend = await firstValueFrom(
              db.friends.populate(["hasFriends"], { shallow: true }).$.get(id)
            );
            expect(
              friend?.hasFriends.every((x) => x instanceof Friend)
            ).toBeTrue();
  ...
  describe("Addon-suite function", () => {
    ...
    it("should not crash on wrong config", async () => {
      expect(() =>
        addonSuite(new Dexie("TestieDb"), { immutable: 34 } as any)
      ).not.toThrow();
    });
    ...
    it("should load encrypted with immutable", async () => {
      ...
      addonSuite(new Dexie("TestieDb"), {
        encrypted: { secretKey: "sdfsdfsfwefwcv" },
      });
      expect(addons).toEqual([
        "immutable",
        "encrypted",
    ...
    it("should load encrypted without immutable", async () => {
      ...
      addonSuite(new Dexie("TestieDb"), {
        encrypted: { secretKey: "sdfsdfsfwefwcv", immutable: false },
      });
      expect(addons).toEqual(["encrypted", "rxjs", "populate", "class"]);
    });
