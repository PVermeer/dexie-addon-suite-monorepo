...
export const testDatabaseNoTableForRelationalKeys = (dexie: typeof Dexie) =>
  new (class TestDatabase extends dexie {
    constructor(name: string) {
      ...
      this.version(1).stores({
        friends:
          "++id, customId, firstName, lastName, shoeSize, age, group => groups.id",
      });
    }
...
export const methodsPositive = [
  // ======== Get() =========
  {
    ...
    method:
      (db: TestDatabaseType) =>
      (_id: number, _shallow = false) =>
        db.friends
          .populate({ shallow: _shallow })
          .get(_id)
          .then((x) => x!),
  },
  {
  ...
  },
  {
    ...
    method:
      ...
      (_id: number, shallow = false) =>
        db.friends
          .populate(["hasFriends", "memberOf", "hairColor"], { shallow })
          .toArray()
          .then((x) => x.find((y) => y.id === _id)),
  },
  {
