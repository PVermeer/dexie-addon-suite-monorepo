...
class TestDatabase extends Dexie {
  constructor(name: string) {
    ...
    this.version(1).stores({
      friends:
        "++id, customId, firstName, lastName, shoeSize, age, hasFriends => friends.id, memberOf => clubs.id, group => groups.id, hairColor => hairColors.id",
...
describe("dexie-populate-addon schema-parser.spec", () => {
  describe("Schema-parser class", () => {
    ...
    describe("Sanitized schema", () => {
      it("should return the correct schema", async () => {
        ...
        const expected = {
          friends:
            "++id, customId, firstName, lastName, shoeSize, age, hasFriends, memberOf, group, hairColor",
          clubs: "++id, name, theme",
