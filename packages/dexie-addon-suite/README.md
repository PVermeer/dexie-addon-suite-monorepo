...
| Symbol | Description                                                       |
| ------ | ----------------------------------------------------------------- |
| \$     | Encrypt this value (_does not create an index_)                   |
...
For individual documentation check the individual repositories / packages (_links are above_).
...
_see individual package readme for more info_
...
- Provide `secretKey` to enable encryption. (_see below and dexie-encrypted-addon on how to use_)
- Provide `immutable: false` to disable immutability. (_not recommended in combination with encryption_)
- Provide `class: false` to disable class mapping. (_class constructor and serializer will not be called anymore_)
