...
export function populate(db: Dexie) {
  ...
  db.on("ready", () => {
    if (
      !dbExt._relationalSchema ||
      !Object.keys(dbExt._relationalSchema).length
    ) {
      console.warn("DEXIE POPULATE ADDON: No relational keys are set");
    }

    if (
      Object.values(dbExt._relationalSchema).some((table) =>
        Object.values(table).some((x) => !db[x.targetTable])
      )
    ) {
      throw new Error(
