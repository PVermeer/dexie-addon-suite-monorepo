...
import { FALSE_STRING, NULL_STRING, TRUE_STRING } from '../../../src/utils';
        ...
        databasesPositive.forEach(database => {
            describe(database.desc, () => {
                ...
                describe('Methods', () => {
                    ...
                    const valueTypes = [
                        { type: 'null', dbValue: NULL_STRING, documentValue: null },
                        { type: 'true', dbValue: TRUE_STRING, documentValue: true },
                        { type: 'false', dbValue: FALSE_STRING, documentValue: false },
                    ] as const;

                    valueTypes.forEach(({ type, dbValue, documentValue }) => {

                        describe(`add() (${type})`, () => {
                            it(`should be "${dbValue}" in db type on create / read`, async () => {
                                ...
                                expect(friendRaw.age === dbValue).withContext('Db value').toBeTrue();
                            });
                        });
                        describe(`bulkAdd() (${type})`, () => {
                            it(`should be "${dbValue}" in db type on create / read`, async () => {
                                ...
                                expect(friendsRaw!.every(friend => friend.age === dbValue)).withContext('Db value').toBeTrue();
                            });
                        });
                        describe(`put() (${type})`, () => {
                            it(`should be "${dbValue}" in db type on create / read`, async () => {
                            ...
                            it(`should be "${dbValue}" in db and null type on put overwrite`, async () => {
                                const [friend] = mockFriends(1);
                        ...
                        describe(`bulkPut() (${type})`, () => {
                            ...
                            it(`should be "${dbValue}" in db on put overwrite`, async () => {
                        ...
                        describe(`update() (${type})`, () => {
                            it(`should be "${dbValue}" in db on update`, async () => {
                                const [friend] = mockFriends(1);
                                friend.age = documentValue;
                                const id = await db.friends.add(friend);
                    ...
                    xdescribe('where()', () => {
                        ...
                        beforeEach(async () => {
                            ...
                            ids = await db.friends.bulkAdd(friends, { allKeys: true });
                            const [nullId] = ids;
                            await db.friends.update(nullId, { age: null, firstName: null });
                            nullFriend = await db.friends.get(nullId) as Friend;
                        });
                        describe('WhereClause', (() => {
                            describe('above()', (() => {
                                it('should not query null values', async () => {
                                    ...
                                    addFriends[3].age = null; // Should be filtered out
                                    ...
                                    expect(getFriends.every(friend => friend.age !== null)).withContext('No null').toBeTrue();
                                    expect(getFriends.length).withContext('Wrong length').toBe(addFriends.length - 1);
                                });
                                it('should query with null on other properties', async () => {
                                    ...
                                    addFriends[3].age = null; // Should NOT be filtered out

                                    addFriends = [...addFriends, addFriendLower, addFriendUpper];
                                    await db.friends.bulkAdd(addFriends);

                                    const getFriends = await db.friends.where('date').above(aboveDate).toArray();
                                    expect(getFriends.some(friend => friend.age === null)).withContext('With null').toBeTrue();
                            ...
                            describe('anyOf()', (() => {
                                it('should be able to query for null (array param)', async () => {

                                    const [addFriend] = mockFriends(1);
                                    addFriend.age = 900;
                                    await db.friends.add(addFriend);

                                    const getFriends = await db.friends.where('age').anyOf([null, 900]).toArray();
                                    expect(getFriends.some(friend => friend.age === null)).withContext('With null').toBeTrue();
                                    expect(getFriends.length).withContext('Wrong length').toBe(2);
                                });
                                it('should be able to query for null (multiple params)', async () => {
                                ...
                                it('should omit null', async () => {

                                    const addFriends = mockFriends(2);
                                    addFriends[0].age = 900;
                                    addFriends[1].age = 800;
                                    await db.friends.bulkAdd(addFriends);

                                    const getFriends = await db.friends.where('age').anyOf(800, 900).toArray();
                                    expect(getFriends.every(friend => friend.age !== null)).withContext('Omit null').toBeTrue();
                                    expect(getFriends.length).withContext('Wrong length').toBe(2);
                                });
                            }));
                            describe('below()', (() => {
                            ...
                            describe('belowOrEqual()', (() => {
                                it('should not query null values', async () => {
                                    ...
                                    addFriends[3].firstName = null; // Should be filtered out

                                    addFriends = [...addFriends, addFriendLower, addFriendUpper];
                                    await db.friends.bulkAdd(addFriends);

                                    const getFriends = await db.friends.where('firstName').belowOrEqual(String.fromCharCode(49)).toArray();
                                    expect(getFriends.every(friend => friend.firstName !== null)).withContext('No null').toBeTrue();
                                    expect(getFriends.length).withContext('Wrong length').toBe(addFriends.length - 1);
                                });
                                it('should query with null on other properties', async () => {
                            ...
                            describe('equals()', (() => {
                                it('should be able to query for null', async () => {

                                    const [getFriend] = await db.friends.where('age').equals(null).toArray();
                                    expect(getFriend).toEqual(nullFriend);
                                });
                            }));
                        }));
