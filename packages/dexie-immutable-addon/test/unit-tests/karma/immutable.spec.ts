...
import {
  ...
  Friend,
  mockFriends,
  TestDatabaseNotImmutable,
  TestDatabaseWithHooks,
            ...
            describe("Overwrite", () => {
              ...
              it("should be able to bulkPut() and get() friends", async () => {
                ...
                const expectedFriends2 = friends2.map((friend, i) => ({
                  ...friend,
                  id: lastId2 - (friends2.length - 1 - i),
                }));

                const getFriends2 = await Promise.all(
                  expectedFriends2.map((friend) => db.friends.get(friend.id))
                );
                expect(getFriends2).toEqual(
                  jasmine.arrayContaining(expectedFriends2)
                );
              });
            });
          });
          ...
          describe("Where()", () => {
            ...
            beforeEach(async () => {
              ...
              friendsWithIds = friendsRead.map((friend, i) => ({
                ...friend,
                id: lastId - (friendsRead.length - 1 - i),
              }));
            });
