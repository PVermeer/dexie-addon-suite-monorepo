...
describe("dexie-rxjs-addon observable-where-clause.class.spec", () => {
  describe("ObservableWhereClause class", () => {
    it("should have new Collection getter", async () => {
      ...
      expect(
        new whereClass.Collection(whereClause) instanceof ObservableCollection
      ).toBeTrue();
      await db.delete();
    });
    it(`should have the correct constructor`, async () => {
      ...
      const whereClass2 = new ObservableWhereClause(
        db,
        db.friends,
        whereClause
      );
