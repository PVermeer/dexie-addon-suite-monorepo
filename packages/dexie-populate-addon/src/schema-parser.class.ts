...
export class SchemaParser {
  ...
  public getRelationalKeys(): RelationalDbSchema {
    return Object.entries(this._schema).reduce<RelationalDbSchema>(
      (acc, [table, value]) => {
        ...
        const relationalKeys = value
          ...
          .reduce<RelationalKeys>((relObj, x) => {
            ...
            const relationalKey = split[0].replace("*", "").replace("&", "");
            const [targetTable, targetKey] = split[1]
              .split(".")
              .map((y) => y.trim());
            return { ...relObj, [relationalKey]: { targetTable, targetKey } };
          }, {});

        if (!Object.keys(relationalKeys).length) {
          return acc;
        }
  ...
  public getCleanedSchema(): StoreSchemas {
    return Object.entries(this._schema).reduce<StoreSchemas>(
      (acc, [table, value]) => {
        if (!value || typeof value !== "string") {
          return { ...acc, [table]: value };
        }
  ...
  constructor(private _schema: StoreSchemas) {}
}
